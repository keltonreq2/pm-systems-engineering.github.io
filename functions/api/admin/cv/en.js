...
export async function onRequestGet({ env }) {
  try {
    ...
    if (!object) return new Response("English CV unavailable", { status: 404, headers: { "Cache-Control": "no-store" } });
    ...
    return new Response("English CV unavailable", { status: 503, headers: { "Cache-Control": "no-store" } });
  }
}
...
export async function onRequestPut({ request, env }) {
  ...
  await env.CV_BUCKET.put(CV_KEY, bytes, {
    httpMetadata: { contentType: "application/pdf", contentDisposition: 'inline; filename="CV-PM-Systems-Engineering-EN.pdf"', cacheControl: "private, no-store" }
  });
  await writeSetting(env.DB, "cv_en_available", "true");
  return json({ ok: true });
}
...
export async function onRequestDelete({ request, env }) {
  ...
  await writeSetting(env.DB, "cv_en_available", "false");
  return json({ ok: true });
}
